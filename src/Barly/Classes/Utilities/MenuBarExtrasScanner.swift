...
/// pushed off-screen by Barly's separator.
...
extension Notification.Name {
    static let barlyWarmHiddenAppsCache = Notification.Name("BarlyWarmHiddenAppsCache")
}
...
final class MenuBarExtrasScanner {
    ...
    /// Returns the AppKit screen frame of Barly's separator NSStatusItem.
    ...
    init() {
        self.warmObserver = NotificationCenter.default.addObserver(
            forName: .barlyWarmHiddenAppsCache,
            object: nil,
            queue: .main
        ) { [weak self] _ in
            Task { @MainActor in
    ...
    private func scheduleRefreshIfNeeded(
        ...
        // Detection only makes sense when Barly is actually hiding items
        // (separator stretched to its expanded length). While Barly is
        // expanded the separator span is small and would misclassify every
        // visible status item as hidden, so we leave the cache untouched.
    ...
    /// Requires Barly to have already expanded the status bar so the item is on-screen.
    @discardableResult
